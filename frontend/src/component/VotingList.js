  ...
  return (
    <Grid style={{ backgroundColor: 'black', minHeight: '100vh'}} container direction="column" alignItems="center">
      <Grid style={{ width:'100%', display:'flex', justifyContent:'flex-end', padding:'2rem' }} item>
        <Button style={{ backgroundColor:'white', color:'black', borderRadius:20, marginRight:'3rem' }} variant="contained" color="primary" onClick={handleCreate}>
          Create Vote
        </Button>
        <Logout/>
      </Grid>
      <Grid item>
        <Tabs value={value} onChange={handleChange} indicatorColor="white">
          ...
          <Tab label="Vote Counting" style={{ color: value === 2 ? 'white' : 'gray', fontWeight: value===2 ? 600:400 }} />
          <Tab label="End Vote" style={{ color: value === 3 ? 'white' : 'gray', fontWeight: value===3 ? 600:400 }} />
        </Tabs>
      </Grid>
      <Grid item>
        {pollList
          ...
          .map((vote) => (
            <Paper
              ...
              sx={{ width: '50vw', mb: 1, p: 5, cursor: 'pointer', borderRadius:5 }}
            >
